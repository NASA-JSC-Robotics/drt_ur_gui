...
from drt_ur_gui.window import ROBOT_MODES


class MockDashboardClient(Node):
    """Imitate dashboard_client node features to test communication with UR.

    Provides dashboard_client services and mimics real dashboard_client responses for testing when the hardware
    dashboard_server is unavailable to connect to. Includes internal simulation of UR arm state that responds to
    service requests.
    ...
    program_name (string): Holds the simulated program name that is currently loaded to the UR
                            ("<unnamed>.urp" on startup)
    ...
    def _set_connect_lag_param(self):
        connect_lag_descriptor = ParameterDescriptor(
            description="Add 6 seconds of lag to the connect service callback to force timeout"
        )
    ...
    def _changeRobotMode(self, target_mode: int):
        ...
        if target_mode == current_mode:  # if robot is already in target mode
            self.get_logger().warning(
                f"Mode change to {ROBOT_MODES[target_mode]} was requested, \
                but robot is already in that mode, skipping..."
            )
            return
        elif target_mode > current_mode:  # Target mode is elevated wrt current mode
            iter_modes = itertools.filterfalse(
                lambda x: x[0] < current_mode or x[0] == RobotMode.BACKDRIVE or x[0] > target_mode,
    ...
    def _initSrvs(self):
        ...
        self.s_addtolog = self.create_service(AddToLog, "~/add_to_log", self.cb_AddToLog, callback_group=self.r_cbg)

        self.s_brakerelease = self.create_service(
            Trigger, "~/brake_release", self.cb_BrakeRelease, callback_group=self.r_cbg
        )

        self.s_clearopmode = self.create_service(
            Trigger, "~/clear_operational_mode", self.cb_ClearOpMode, callback_group=self.r_cbg
        )

        self.s_closepopup = self.create_service(Trigger, "~/close_popup", self.cb_ClosePopup, callback_group=self.r_cbg)

        self.s_closesafetypopup = self.create_service(
            Trigger, "~/close_safety_popup", self.cb_CloseSafetyPopup, callback_group=self.r_cbg
        )

        self.s_connect = self.create_service(Trigger, "~/connect", self.cb_Connect, callback_group=self.r_cbg)

        self.s_getloadedprogram = self.create_service(
            GetLoadedProgram, "~/get_loaded_program", self.cb_GetLoadedProgram, callback_group=self.r_cbg
        )

        self.s_getrobotmode = self.create_service(
            GetRobotMode, "~/get_robot_mode", self.cb_GetRobotMode, callback_group=self.r_cbg
        )

        self.s_getsafetymode = self.create_service(
            GetSafetyMode, "~/get_safety_mode", self.cb_GetSafetyMode, callback_group=self.r_cbg
        )

        self.s_loadinstallation = self.create_service(
            Load, "~/load_installation", self.cb_LoadInstallation, callback_group=self.r_cbg
        )

        self.s_loadprogram = self.create_service(Load, "~/load_program", self.cb_LoadProgram, callback_group=self.r_cbg)

        self.s_pause = self.create_service(Trigger, "~/pause", self.cb_Pause, callback_group=self.r_cbg)

        self.s_play = self.create_service(Trigger, "~/play", self.cb_Play, callback_group=self.r_cbg)

        self.s_popup = self.create_service(Popup, "~/popup", self.cb_Popup, callback_group=self.r_cbg)

        self.s_poweroff = self.create_service(Trigger, "~/power_off", self.cb_PowerOff, callback_group=self.r_cbg)

        self.s_poweron = self.create_service(Trigger, "~/power_on", self.cb_PowerOn, callback_group=self.r_cbg)

        self.s_programrunning = self.create_service(
            IsProgramRunning, "~/program_running", self.cb_ProgramRunning, callback_group=self.r_cbg
        )

        self.s_programsaved = self.create_service(
            IsProgramSaved, "~/program_saved", self.cb_ProgramSaved, callback_group=self.r_cbg
        )

        self.s_programstate = self.create_service(
            GetProgramState, "~/program_state", self.cb_ProgramState, callback_group=self.r_cbg
        )
        ...
        self.s_rawrequest = self.create_service(
            RawRequest, "~/raw_request", self.cb_RawRequest, callback_group=self.r_cbg
        )

        self.s_restartsafety = self.create_service(
            Trigger, "~/restart_safety", self.cb_RestartSafety, callback_group=self.r_cbg
        )

        self.s_shutdown = self.create_service(Trigger, "~/shutdown", self.cb_Shutdown, callback_group=self.r_cbg)

        self.s_stop = self.create_service(Trigger, "~/stop", self.cb_Stop, callback_group=self.r_cbg)

        self.s_unlockpstop = self.create_service(
            Trigger, "~/unlock_protective_stop", self.cb_UnlockPStop, callback_group=self.r_cbg
        )
